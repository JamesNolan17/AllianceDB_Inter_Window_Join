...
def ReadFile(id):
    ...
    for i in range(id, bound, 1):
        ...
        for line in read:
            ...
            if len(field) > 1:
                if field[1] == "Retiring":
                    counters["retiring"] = float(field[2])
                elif field[1] == "Bad Speculation":
                    counters["badspec"] = float(field[2])
                elif field[1] == "Front-End Bound":
                    counters["frontendbd"] = float(field[2])
                elif field[1] == "Memory Bound":
                    counters["memorybd"] = float(field[2])
                elif field[1] == "Core Bound":
                    counters["cpubd"] = float(field[2])
        y[0][j] = counters["retiring"]
        y[1][j] = counters["badspec"]
        y[2][j] = counters["frontendbd"]
        y[3][j] = counters["memorybd"]
        y[4][j] = counters["cpubd"]
        j += 1
        if j == 4:
            y[0][j] = 0
            y[1][j] = 0
...
if __name__ == "__main__":
    ...
    # break into 4 parts
    legend_labels = ['retiring', 'bad speculation', 'frontend bound', 'memory bound', 'cpu bound']  # , 'others'
