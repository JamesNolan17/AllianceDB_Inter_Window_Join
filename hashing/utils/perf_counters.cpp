...
void PCM_initPerformanceMonitor(const char *pcmcfg, const char *pcmout) {
  ...
  if (PCM_CONFIG) {
    ...
    while (!inpf.eof() && numEvents < 4) {
      inpf >> MyEventNames[numEvents] >> hex >>
          MyEvents[numEvents].event_number >> hex >>
          MyEvents[numEvents].umask_value;
      numEvents++;
    }
    inpf.close();
  ...
  case PCM::MSRAccessDenied:
    MSG("Access to Intel(r) Performance Counter Monitor has denied (no MSR or "
        "PCI CFG space access).")
    break;
  case PCM::PMUBusy:
    MSG("Access to Intel(r) Performance Counter Monitor has denied "
        "(Performance Monitoring Unit is occupied by other application). Try "
        "to stop the application that uses PMU.")
    MSG("Alternatively you can try to reset PMU configuration at your own "
        "risk. Try to reset? (y/n)")
...
void PCM_printResults() {
  ...
  (*out) << "Active-cycles " << getCycles(before_state, after_state) << endl;
  (*out) << "IPC " << getIPC(before_state, after_state) << endl;

  if (numEvents == 0) {
    (*out) << "L2Hit " << getL2CacheHits(before_state, after_state) << endl;
    (*out) << "L2Misses " << getL2CacheMisses(before_state, after_state)
           << endl;
    (*out) << "L3Misses " << getL3CacheMisses(before_state, after_state)
           << endl;
    (*out) << "L2HitRatio " << getL2CacheHitRatio(before_state, after_state)
           << endl;
    ...
    (*out) << "BytesWrittenToMC "
           << getBytesWrittenToMC(before_state, after_state) << endl;
#endif
  }

  for (int i = 0; i < numEvents; i++) {
    (*out) << MyEventNames[i] << " "
           << getNumberOfCustomEvents(i, before_state, after_state) << endl;
  }
...
void PCM_printAccumulators() {
  ...
  if (numEvents == 0) {
    ...
    (*out) << "CyclesLostDueL2CacheMisses " << eventAcc[7] << endl;
    (*out) << "CyclesLostDueL3CacheMisses " << eventAcc[8] << endl;
